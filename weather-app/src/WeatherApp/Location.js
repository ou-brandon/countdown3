...
const LocationInput = (props) => {
    ...
    useEffect(()=>{
        if(zip && countryCode){
            const url = "http://api.openweathermap.org/geo/1.0/zip?zip=" + zip + ',' + countryCode + '&appid=' + API_KEY;
            setAPI(url);
            fetch(API)
            .then((res) => res.json())
            .then((res) => {
        ...
        if(city && state && countryCode){
          const url = "http://api.openweathermap.org/geo/1.0/direct?q=" + city + ',' + state + ',' + countryCode + '&limit=1' + '&appid=' + API_KEY;
          console.log("asdf");
          setAPI(url);
          fetch(API)
          .then((res) => res.json())
    ...
    }, [city, state, zip, countryCode, API, API_KEY])
          ...
          <TabPanel value={value} index={0}>
              <div className='uncover-tab'>
                  <div className='sidebar'>
                      ...
                      <br></br>
                      <br></br>
                      <TextField color='secondary' sx={{ input: {color: 'white'}}} label="City Name" variant="outlined" defaultValue={city} inputRef={cityRef} InputLabelProps={{style: {color: 'white'}}}/>
                      <br></br>
                      <br></br>
                      <TextField color='secondary' sx={{ input: {color: 'white'}}} label="State Code (USA Only)" variant="outlined" defaultValue={state} inputRef={stateRef} InputLabelProps={{style: {color: 'white'}}} />
                      <br></br>
                      <br></br>
